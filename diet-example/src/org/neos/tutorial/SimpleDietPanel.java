...
public class SimpleDietPanel extends JPanel implements ActionListener {
	...
	public void createGUI() {
		...
		// --------------------------------------------------------
		// Uncomment these lines to add Food selection panel
...
//		add(selectionPanel, BorderLayout.CENTER);
	...
	String getSelection() {
		// --------------------------------------------------------
		// Declare set name and description
		
		Set fs = new Set("name", "Description");
		for (SimpleFood food : foodList) {
			
			// Add selected food into the set	
	...
	void submitJob() {
		...
		// --------------------------------------------------------
		// Modify here to get the user selection instead of this list
		
		String selected = "Set fs(f) foods selected  / Bro1,Chi66,Por50,Pot7,Tor56 /;";
		String combinedModel = dataDomain + selected + model;
		...
		NeosJob job = client.submitJob(dietJob.toXMLString());
		System.out.println(job.getResult());
		
		// --------------------------------------------------------
		// Component 2 lines above and uncomment these lines to launch job popup
