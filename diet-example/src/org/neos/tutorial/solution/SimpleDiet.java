...
public class SimpleDiet {
	...
	private static final boolean SOLUTION_PARSER = true;
	private static final boolean RESULTS_TXT = true;
	...
	public static void main(String[] args) {
		...
		NeosJobXml job = new NeosJobXml("nco", "MINOS", "GAMS");
		job.addParam("model", model);
		...
		if (SOLUTION_PARSER) {
			...
			if (parser.getModelStatusCode() == 1) {
				...
				// buy = null;
				// cost = null;
				buy = parser.getSymbol("buy", SolutionData.VAR, 1);
				...
				if (buy != null) {
					...
					for (SolutionRow row : buy.getRows()) {
						...
						String foodName = row.getIndex(0);
						Double level = row.getLevel();
						Double marginal = row.getMarginal();
						// -------------------------------------------------------------------
